...
{
    let t = ""
    ths = "Nome,cargo,email,Ações".split(",")
    for (let i of ths)
    ...
    for (let i of lista)
    {
        ...
        t += "<a href='funcionarios/consultar/" + i.id + "'>Ver</a>"
        t += "<a href='funcionarios/editar/" + i.id + "'>Editar</a>"
        t += "</div>"
    }
...
{
    console.log(5)
    lista = getJSON("api/fun/list.php?" + document.getElementById("status").selectedIndex)
    montar()
}
function pesquisar()
{
...
function gerarPdf()
{
    ...
    for (let i of lista)
        array.push(
            [i.nome, i.cargo, i.email, ((i.status == 1) ? "Ativo" : "Desativado")]
        )
